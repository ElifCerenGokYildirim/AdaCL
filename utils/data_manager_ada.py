...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment):
        ...
        self.val_data, self.val_targets = [], []
        self.train_indx = []
    ...
    def get_dataset(
        self, indices, source, mode, appendents=None, ret_data=False, m_rate=None
    ):
        if source == "train":
        ...
        for idx in indices:
            ...
            if mode == "train":
                data.append(class_data[self.train_indx[idx]])
                targets.append(class_targets[self.train_indx[idx]])
            elif mode == "test":
        ...
        if appendents is not None:
            for appendent in appendents:
                #print("appendent:", appendent)
                #print("---------------------------")
                if len(appendent[1]) != 0:
    ...
    def get_val_dataset(
        self, indices, source, mode, val_samples_per_class=0
    ):
        if source == "train":
            x, y = self._train_data, self._train_targets
        else:
        ...
        for idx in indices:
            ...
            self.train_indx.append(train_indx)
            self.val_data.append(class_data[val_indx])
            self.val_targets.append(class_targets[val_indx])

        val_data, val_targets = np.concatenate(self.val_data), np.concatenate(self.val_targets)
